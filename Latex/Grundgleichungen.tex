...
Innerhalb dieses Abschnittes wird angenommen, die unbekannte Koordinate sei die z-Komponente.
...
\begin{tikzpicture}
	...
	%Normale
	\coordinate (offSpitze) at (1.2, 1.8);
	\coordinate (nSpitze) at ($(q) +(offSpitze)$);
	\coordinate (hn) at ($(q)!0.3cm!(nSpitze)$) {};
	\coordinate (hl) at  ($(q)!0.3cm!(q'l)$){};
	\coordinate (hm) at ($(q)!0.3cm!(q'm)$) {};

	\coordinate (hilfspkt) at ($(hl) +(offSpitze)$);
	\coordinate (hlo) at  ($(hl)!0.3cm!(hilfspkt)$){};
	\draw[white, line width=3pt] (hn) -- (hlo);
	\draw[purple, thin] (hn) -- (hlo);
	\draw[purple, thin] (hl) -- (hlo);

	\coordinate (hilfspkt) at ($(hm) +(offSpitze)$);
	\coordinate (hmo) at  ($(hm)!0.3cm!(hilfspkt)$){};
	\draw[white, line width=3pt] (hn) -- (hmo);
	\draw[purple, thin] (hn) -- (hmo);
	\draw[purple, thin] (hm) -- (hmo);

	\draw[purple, thick, -Latex] (q) -- node[sloped, anchor=south, xslant=0.533, fill=white]{$\vec{n}$} (nSpitze);
\end{tikzpicture}
\end{figure}
