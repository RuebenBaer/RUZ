...
void RUZmBIFrame::HoehenkarteZeichnen(thread_info_integral *thInf)
{
	if(aktLayer)
	{
		...
		if(aktLayer->AusdehnungFinden(minX, minY, maxX, maxY, minZ, maxZ))
		{
			...
			SetStatusText(wxT("Integration erledigt - Grenzen ermitteln"), 1);
			Refresh();
			for(int i = 0; i < iB; i++)
			{
				for(int k = 0; k < iH; k++)
			...
			if((oleX < ureX)&&(oleY < ureY))
			{
				...
				if(iB!=0 && iH!=0)
				{
					...
					if(iB!=0 && iH!=0)
					{
						...
						dc.DrawBitmap(wxBitmap(imBild, dc), (lwBild.dOffsetX - dc_Offset[0])*m_skalierung + (int)(oleX)*m_skalierung,
							(lwBild.dOffsetY - dc_Offset[1])*m_skalierung + (int)(oleY)*m_skalierung); // Problematische Zeile !!!
					}
				}
			}
		}
