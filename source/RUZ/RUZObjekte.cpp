...
{
    m_vA = p[1]->HolePosition()-p[0]->HolePosition();
...
bool Viereck::LageAufFlaeche(Vektor& vOrt, RelativLage& rl_Lage_1, RelativLage& rl_Lage_2, Achse projRichtung)
{
    bool bErfolg = false;
    Vektor d, e;
    d = Vektor(0.0, 0.0, 0.0);
    d.SetKoordinaten(projRichtung, 1.0);
    e = vOrt - p[0]->HolePosition();
...
void Viereck::HoleNormale(Vektor& punktQ, Vektor& vNormale, Achse prjRichtung)
{
    ...
    {
        vNormale = (m_vB + m_vC * relLage1.mue); //Kreuzprodukt auf zwei operationen verteilt (const!)
        vNormale = vNormale.Kreuz(m_vA + m_vC * relLage1.lambda);
...
bool Viereck::OrtAufFlaeche(Vektor& punktQ, Achse prjRichtung)
{
    ...
    if(DurchstossPunkt(punktQ, punktQ2, punktQ, vErgebnis2))
    {
        return true;
    }
...
bool Viereck::DurchstossPunkt(Vektor v_Ln_start, Vektor v_ln_ende, Vektor& v_rueckgabe1, Vektor& v_rueckgabe2, bool aussenErlaubt)
{
    ...
    v_rueckgabe1 = p[0]->HolePosition() + m_vA * mue1 + m_vB * lambda1 + m_vC * mue1 * lambda1;
    v_rueckgabe2 = p[0]->HolePosition() + m_vA * mue2 + m_vB * lambda2 + m_vC * mue2 * lambda2;
    return bErfolg;
}
...
void Viereck::ImLogAusgeben(void)
{
    ...
    logSchreiben("m_vA:\t%.5f, \t%.5f, \t%.5f\n", m_vA.x(), m_vA.y(), m_vA.z());
    logSchreiben("m_vB:\t%.5f, \t%.5f, \t%.5f\n", m_vB.x(), m_vB.y(), m_vB.z());
    logSchreiben("m_vB:\t%.5f, \t%.5f, \t%.5f\n", m_vC.x(), m_vC.y(), m_vC.z());
    return;
}
/*ENDE Viereck*/
