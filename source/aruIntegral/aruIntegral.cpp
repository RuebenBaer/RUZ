...
void aruIntegral::IntegriereFlaeche(Dreieck *obj)
{
	...
	for(int i = 0; i < 3; i++)
	{
		ZeichneLinie(px[i], py[i], px[(i+1)%3], py[(i+1)%3], obj);
	}
	return;
}
...
void aruIntegral::IntegriereFlaeche(Viereck *obj)
{
	...
	for(int i = 0; i < 4; i++)
	{
		ZeichneLinie(px[i], py[i], px[(i+1)%4], py[(i+1)%4], obj);
	}
	return;
}
...
void aruIntegral::ZeichneTrapezSenkrecht(int iMinX, int iMaxX, int iMinYu, int iMaxYu, int iMinYo, int iMaxYo, Flaeche* obj)
{
	if(iMaxX == iMinX)
	{
		ZeichneLinie(iMinX, iMinYu, iMaxX, iMaxYo, obj);
		return;
	}
...
void aruIntegral::ZeichneLinie(int p1x, int p1y, int p2x, int p2y, Flaeche* obj)
{
	...
	if(abs(dx) < abs(dy))
	{
		...
		for(y = yMin; y < yMax; y++)
		{
			...
			if((x < iBreite)&&(x >= 0))
			{
				tempOrt.SetKoordinaten((aProjektion+1)%3, (x + iOffsetBreite)*dAufloesung);
				tempOrt.SetKoordinaten((aProjektion+2)%3, (y + iOffsetHoehe)*dAufloesung);
				tempOrt.SetKoordinaten((aProjektion+0)%3, NAN);
				if(obj->OrtAufFlaeche(tempOrt, aProjektion, true))
				{
					dIntegral[x + y * iBreite] = tempOrt.GetKoordinaten(aProjektion%3);
				}
			}
		}
	}else
